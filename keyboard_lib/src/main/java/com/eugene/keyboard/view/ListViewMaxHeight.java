package com.eugene.keyboard.view;

import android.content.Context;
import android.content.res.TypedArray;
import android.util.AttributeSet;
import android.widget.ListView;
import com.eugene.keyboard.R;

/**
 * Created by eugene on 19/08/2016.
 */
public class ListViewMaxHeight extends ListView {
    ...
    public ListViewMaxHeight(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray a =
                    getContext().obtainStyledAttributes(attrs, R.styleable.ListViewMaxHeight);
            maxHeight = a.getDimensionPixelSize(R.styleable.ListViewMaxHeight_maxHeight,
                    Integer.MAX_VALUE);
            a.recycle();
        } else {
            maxHeight = 0;
